...
class MainPage(BasePage):
    ...
    def scroll_into_requests(self):
        element = self.browser.find_element(
            *MainPageLocators.SECTION_WITH_ENDPOINTS
        )
    ...
    def is_all_endpoint_elements_are_visible(self, method, request_locator):
        if method == 'get' or method == 'delete':
            assert self.is_element_present(*request_locator) and \
                self.is_element_present(*MainPageLocators.REQUEST_URL) \
                and self.is_element_present(*MainPageLocators.RESPONSE_CODE) \
                and self.is_element_present(
                *MainPageLocators.OUTPUT_RESPONSE
            ), "Не все поля запроса отображены"
        else:
            assert self.is_element_present(*request_locator) and \
                self.is_element_present(*MainPageLocators.REQUEST_URL) \
                and self.is_element_present(*MainPageLocators.RESPONSE_CODE) \
                ...
                *MainPageLocators.OUTPUT_RESPONSE
                ...
                and self.is_element_present(
                *MainPageLocators.OUTPUT_REQUEST
            ), "Не все поля запроса отображены"
    ...
    def is_response_code_equal(self, api_response_code: str):
        ui_response_code = self.get_element(
            *MainPageLocators.RESPONSE_CODE
        ).text
        assert api_response_code == (
            ui_response_code,
            "Коды ответа не совпадают"
        )

    def is_response_body_equal(self, method: str, api_response_body):
        ui_response_body = loads(
            self.get_element(*MainPageLocators.OUTPUT_RESPONSE).text
        )
        if method == 'get':
            assert api_response_body == (
                ui_response_body,
                'Тела ответа не совпадают'
            )
        else:
            assert api_response_body.keys() == (
                ui_response_body.keys(),
                'Поля ответов не совпадают'
            )
